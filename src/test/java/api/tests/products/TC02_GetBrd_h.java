...
public class TC02_GetBrd_h extends ApiTestBase {
    @Test
    public void test() {
        // 1. Construct API Request (Headers, Params) and Send HTTP Request
        Response res = given().spec(ApiBase.getFrmReq()).when().get(ApiRts.brndsList_url);
        ...
        Assert.assertTrue(res.getBody().asString().contains("brands"));
