...
public class TC41_CrtBlk_n extends ApiTestBase {
    @Test
    public void test() {
        // 1. Construct API Request (Headers, Params) and Send HTTP Request
        Assert.assertEquals(given().spec(ApiBase.getFrmReq()).formParam("name","").when().post(ApiRts.crtAcc_url).jsonPath().getString("responseCode"), "400");
