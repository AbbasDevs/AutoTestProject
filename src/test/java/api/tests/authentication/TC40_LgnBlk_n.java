...
public class TC40_LgnBlk_n extends ApiTestBase {
    @Test
    public void test() {
        // 1. Construct API Request (Headers, Params) and Send HTTP Request
        Assert.assertEquals(given().spec(ApiBase.getFrmReq()).formParam("email","").when().post(ApiRts.vrfyLgn_url).jsonPath().getString("responseCode"), "400");
