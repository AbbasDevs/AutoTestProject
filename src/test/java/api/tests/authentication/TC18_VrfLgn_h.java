...
public class TC18_VrfLgn_h extends ApiTestBase {
    @Test
    public void test() {
        // 1. Construct API Request (Headers, Params) and Send HTTP Request
        Response res = given().spec(ApiBase.getFrmReq()).formParam("email", usrPl.getEmail()).formParam("password", usrPl.getPassword()).when().post(ApiRts.vrfyLgn_url);
