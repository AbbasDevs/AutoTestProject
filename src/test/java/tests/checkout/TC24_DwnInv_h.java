...
public class TC24_DwnInv_h extends TestBase {
	HomePage homeObject;
	CartPage cartObject;
	...
	public void downloadInvoice(String name, String card, String cvc, String month, String year) throws InterruptedException {
		...
		checkoutObject.dwnInvBtn.click();
		Thread.sleep(3000); // Give chrome time to download the file
		
		File invoiceFile = new File(System.getProperty("user.home") + "\\Downloads\\invoice.txt");
		Assert.assertTrue(invoiceFile.exists(), "Invoice file was not downloaded!");
		invoiceFile.delete(); // Clean up!
